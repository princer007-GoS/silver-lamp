...
-- RETURN IF LOADED
if (_G.GamsteronPredictionModLoaded) then
    return
end
_G.GamsteronPredictionModLoaded = true
...
do
    ...
    local scriptName = "GamsteronPredictionMod"
    local success, newVersion = AutoUpdate({
        version = SCRIPT_VERSION,
        scriptPath = COMMON_PATH .. scriptName .. ".lua",
        scriptUrl = "https://raw.githubusercontent.com/princer007-GoS/silver-lamp/master/Common/" .. scriptName .. ".lua",
        versionPath = COMMON_PATH .. scriptName .. ".version",
        versionUrl = "https://raw.githubusercontent.com/princer007-GoS/silver-lamp/master/Common/" .. scriptName .. ".version"
    })
    if (success) then
...
-- MENU
local Menu = MenuElement({name = "Gamsteron Prediction Mod", id = "GamsteronPredictionMod", type = _G.MENU})
...
function GetGamsteronPrediction(unit, args, source)
    ...
    local spellIsCastSource = false
    if (args.SpellIsCastSource ~= nil) then
        spellIsCastSource = args.SpellIsCastSource
    end
    -- output
    ...
    if inputRange ~= math.huge then
		...
		if spellIsCastSource then mepos = source.pos end
...
do
    ...
    local __GamsteronPredictionMod = LocalClass()

    function __GamsteronPredictionMod:__init()
    end

    function __GamsteronPredictionMod:GetPrediction(unit, args, source)
        return GetGamsteronPrediction(unit, args, source)
    end

    _G.GamsteronPredictionMod = __GamsteronPredictionMod()
end
